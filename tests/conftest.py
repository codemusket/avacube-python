...
from avacube import Avacube, AsyncAvacube, DefaultAioHttpClient
from avacube._utils import is_dict
...
logging.getLogger("avacube").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Avacube]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Avacube(base_url=base_url, auth_key=auth_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncAvacube]:
    ...
    async with AsyncAvacube(
        base_url=base_url, auth_key=auth_key, _strict_response_validation=strict, http_client=http_client
    ) as client:
        yield client
